...
verbose = 0 #print for debug
lang_path =  os.path.dirname(__file__) + "/lang"
lang = ConfigManager.getConfiguration()['lang_short']
_ = NeoTrans(domain = 'neotique', localedir = lang_path, languages = [lang]).Trans
...
class NeoConv():
    ...
    def WITDate(cls, pjson=""):
        ...
        dDate['tmonth'] = _(dDate['month'])
        dDate['day'] = dfrom.date().strftime("%c")[:3]
        dDate['tday'] = _(dDate['day'])
        ...
        if dfrom.time().hour == 18 and dto.time().hour == 00: #speclial case evening for WIT
            ...
            dDate['part'] = "alltheday"
        dDate['tpart'] = _(dDate['part'])

        return dDate

    #-----------------------------------------------------------------------------
    @classmethod
    def time2str(cls, pTime, pMinutes = 1, pSecondes = 0):
        """
        ...
        """
        if verbose  == 1 : print  'input     :',pTime, type(pTime)
        ...
        if pMinutes == 1:
            ...
            if pSecond == 1:
                s = pTime.strftime("%S")
                msg += _('and') + s + _('second')

        if verbose  == 1 : print 'time2str output    :',msg
        return msg
